...
//! Convenience helpers for planning Solace Platform sources and sinks from SQL.
...
pub const DEFAULT_ACK_WINDOW_SIZE: u32 = 255;
...
pub const DEFAULT_FLOW_MAX_UNACKED: i32 = 10_000;
...
/// Default consumer worker count. The runtime clamps ingestion to a single
/// worker, so values above 1 are rejected at planning.
...
generate_extracted_config!(
    ...
    (AckWindowSize, u32, Default(DEFAULT_ACK_WINDOW_SIZE)),
    (FlowMaxUnacked, i32, Default(DEFAULT_FLOW_MAX_UNACKED)),
    (Deduplicate, bool, Default(DEFAULT_DEDUPLICATE)),
    (Parallelism, u32, Default(DEFAULT_PARALLELISM)),
